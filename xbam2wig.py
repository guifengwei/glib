...
Copyright @ Guifeng Wei 2017 - 2018
'''
...
## 0.1.6 adjust the normalization factor 2017-06-15
## 0.1.7 the read shift to the isize, not to 200 nt as before, just the reads fragment size as did in experiment 2017-09-15
## Please keep in mind that now the script suits for the paired-end ChIP-seq only
...
class BamBins:
    ...
    def readBam(self):
        ...
        for s in self.samfile:
            i+=1
            if i%100000==0: print >>sys.stderr,i,"read mate\r",
            if s.tid==-1:
                self.unmapped+=1
                continue
            self.mapped+=1
            ...
            if s.is_reverse:
                ...
                ### fragment length, isize
                ### use fragment length rather than shifting the reads to one nucelosome lenght or 200 nt
                start,stop=s.pos,s.pos+s.isize
        ...
        if norm:
            ## conventional normalization method, to 1 million mapped reads
            libsize = self.mapped*0.5/1000000
            ## calibrate normalization method, to the desired libsize, i.e. customerize lib size
            ### 1.000001 is 1 million mapped reads, this number is system-keep number for normalizaiton
            if norm != 1.000001:
                libsize = norm
            for chrs in self.chrs:
                ...
                for bin_id,value in enumerate(self.bins[self.chr2tid[chrs]]):
                    ...
                    # paired-end reads, total reads = self.mapped/2
                    # norm is a parameter to control how large the library size is
                    self.bins[self.chr2tid[chrs]][bin_id] = (1.0 * self.bins[self.chr2tid[chrs]][bin_id])/libsize
...
def parse_argument():
    '''
        argument parser
    '''
    p=argparse.ArgumentParser(description='example: %(prog)s -b binsize -C input.bam --bam chip.bam -w output.wig',epilog='dependency pysam')
    # Binsize
    ...
    p.add_argument('-n', "--norm", dest='norm', type=float, metavar="Norm", default=1.000001, help="The library size. Default: Normalized to 1 million mapped reads, If you performed Calibrated ChIP-seq, you can set it to the total mapped spike-in library size (for example, if you used spike-in chromatin and got 1,500,000, you can set it 1.5)")
...
def main():
    ...
    print >>sys.stderr,"# xbam2wig.py 0.1.7 "
